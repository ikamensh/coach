...
//
// IMPORTANT: Cursor's hook runner silently rejects any hook command whose
// shell-form mentions `curl` (and most likely other network binaries) as
// part of an apparent denylist. We confirmed empirically that:
//   - `touch /tmp/X`               → fires
//   - `which curl > /tmp/X`        → fires (curl is just an arg to which)
//   - `touch /tmp/before; curl X`  → does NOT fire — even the touch is dropped
//   - a wrapper shell script that internally calls curl → fires fine
//
// So we can't put `curl` directly in `hooks.json`. Instead we install a
// tiny shim script (`coach-cursor-hook.sh`, sibling of hooks.json) and
// reference it from each hook entry. The shim takes the event slug as $1,
// reads JSON on stdin, and POSTs it to the local Coach HTTP server.
// Hook command entries become `<shim path> <event-slug>`, which Cursor
// happily executes.
...
/// Body of the shim script for the given Coach port. Single source of
/// truth for what `install_cursor_hooks_at` writes to disk and what tests
/// can reproduce. Uses `--data-binary @-` so stdin is forwarded byte-for-
/// byte to the request body, and `exec` so the curl exit code becomes the
/// shim's exit code (cursor hooks treat non-zero as errors).
pub fn cursor_shim_script(port: u16) -> String {
    format!(
        "#!/bin/sh\n\
         # Auto-generated by Coach. Forwards a Cursor hook payload (read from\n\
         # stdin) to the local Coach HTTP server. Installed because Cursor's\n\
         # hook runner blocks hook commands that reference `curl` directly.\n\
...
/// Hook command entries Coach installs for each Cursor lifecycle event.
/// Each entry is `<shim_path> <event-slug>`. Cursor's hook runner spawns
/// these via the user's shell with the event JSON piped on stdin.
...
/// script by filename, OR if it's a legacy `curl ... /cursor/hook/...`
/// entry from a pre-shim install. We match on filename rather than full
/// path so a stale install at a different shim path is still recognized
/// and cleaned up by uninstall, and the legacy match lets reinstall sweep
/// away the inert curl-based entries Cursor was silently dropping.
fn is_managed_cursor_command(cmd: &str) -> bool {
    cmd.contains(CURSOR_SHIM_FILENAME) || cmd.contains("/cursor/hook/")
}

pub fn check_cursor_hook_status(port: u16) -> HookStatus {
    let _ = port; // shim path is now the only thing check needs
...
pub fn install_cursor_hooks_at(
    ...
    // 1. Write the shim script (chmod +x on Unix). Done first so the
    //    referenced executable exists by the time hooks.json is on disk.
