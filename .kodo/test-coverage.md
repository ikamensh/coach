...
**Linux Stage 4 (daemon HTTP + PID + load, 2026-04-10):** Independent testers on **Debian 12 ARM64** (`root@46.225.111.102`) with release binary **`/root/coach/target/release/coach`** (0.1.78/0.1.79 per synced build). Isolated **`mktemp` `HOME`**: **`coach serve`**, realistic **`curl`** hook lifecycles (Claude **`/hook/…`**, Cursor/Codex routes, **`/api/state`**), **`coach status`**, clean shutdown (Ctrl-C/SIGTERM). **`ss -tnp`** during in-flight hook POSTs showed client **`curl`** PID + ephemeral port matching daemon logs (`resolved sid … → pid … (peer port …)`), confirming the Linux PID resolver. Concurrent hook stress (hundreds of POSTs) returned **200** for completed requests; `/api/state` stable across polls; shutdown under load freed the listen port (requests racing teardown failed at TCP as expected). **Session model:** rows keyed by **resolved OS PID**; **`session_id`** is stored per row — counts reflect process identity, not “unique short session_id strings” (by design). Full repro: **`.kodo/tester-notes.md`** (Linux ARM64 → PID resolution + concurrent stress).
